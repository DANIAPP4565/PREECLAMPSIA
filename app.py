...
def demographic_form(defaults: Dict[str, Any], key_prefix: str = "demo") -> Dict[str, Any]:
    ...
    with c2:
        ...
        edad = st.number_input("Edad materna", min_value=10, max_value=60, value=int(clean_num(defaults.get("edad")) or 30), step=1, key=f"{key_prefix}_edad")
    with c3:
        eg = st.number_input("Edad gestacional (semanas)", min_value=5.0, max_value=42.0, value=float(clean_num(defaults.get("edad_gestacional")) or 24.0), step=0.1, key=f"{key_prefix}_eg")
        obra_social = st.text_input("Obra social", value=str(defaults.get("obra_social", "")), key=f"{key_prefix}_obra_social")
    with c4:
        pas = st.number_input("PAS clínica", min_value=70, max_value=260, value=int(clean_num(defaults.get("PAS")) or 120), step=1, key=f"{key_prefix}_pas")
        pad = st.number_input("PAD clínica", min_value=40, max_value=160, value=int(clean_num(defaults.get("PAD")) or 80), step=1, key=f"{key_prefix}_pad")
    return {
